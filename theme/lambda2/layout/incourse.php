...
$haspreblocks = ((strpos($preblockshtml, 'data-block=') !== false || !empty($addblockbutton)) && (strpos($preblockshtml, 'block-fake') == false));
if (!$haspreblocks) {
    $blockdraweropen = false;
}
...
$is_pagetype_enrol = $PAGE->pagetype === 'enrol-index';
$show_sharebuttons_enrol = theme_lambda2_get_setting('sharebuttons_enrol');
$show_sharebuttons_sitenews = theme_lambda2_get_setting('sharebuttons_sitenews');
$showbacktocoursebtn = (theme_lambda2_get_setting('back_to_course') && !$is_pagetype_enrol);
...
$mainregionblocks = false;
if ((theme_lambda2_get_pagetype() == "mod-page-view") || (theme_lambda2_get_pagetype() == "mod-book-view")) {
    $mainregionblocks = true;
}
...
if (theme_lambda2_get_setting('footnote', true) == true) { 
    $footnote = theme_lambda2_get_setting('footnote');
    $hasfootnote = true;
}
...
$templatecontext = [
    ...
    'mainregionblocks' => $mainregionblocks,
    ...
    'showbacktocoursebtn' => $showbacktocoursebtn,
    'is_pagetype_enrol' => $is_pagetype_enrol,
    'show_sharebuttons_enrol' => $show_sharebuttons_enrol,
    'show_sharebuttons_sitenews' => $show_sharebuttons_sitenews,
...
$custom_enrolment_page = theme_lambda2_check_enrolment();
$site_post = theme_lambda2_check_site_announcements();
if ($custom_enrolment_page == 2) {
    echo $OUTPUT->render_from_template('theme_lambda2/enrol_lambda_2', $templatecontext);
} else if ($custom_enrolment_page == 3) {
    echo $OUTPUT->render_from_template('theme_lambda2/enrol_lambda_3', $templatecontext);
} else if ($custom_enrolment_page == 4) {
    echo $OUTPUT->render_from_template('theme_lambda2/enrol_lambda_4', $templatecontext);
} else if($site_post) {
    echo $OUTPUT->render_from_template('theme_lambda2/site_post', $templatecontext);
} else {
    echo $OUTPUT->render_from_template('theme_lambda2/incourse', $templatecontext);
}